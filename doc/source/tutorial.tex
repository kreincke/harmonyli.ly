...
score. The benefit is that now \lily\ itself aligns the music notes and the
respective analysis symbols: it prevents horizontal overlappings, even if a
\has\ is much longer than the respective note.

The little disadvantage is that in this case \hlyn\ requires a dedicated
voice to which the row of \has{s} can be bound. After having set up your
\lily\ file as described above you have three opportunities to do so:
...
attributes. Let us visualize the structure of a \has{}\ by using our own
function \texttt{\textbackslash setHas}
...
\include "lilypond/harmonyli.ly"
\score {
  \new Staff { \clef "treble" \key d \major \time 4/4 \stemUp < fis' a' d''>1 }
  \addlyrics { 
    \markup 
...
\subsection{Inserting the Appropriate Function Symbol}
...
a scale are referred by their harmonical functions (represented by characters).\footcite[for
dedails cf.][\nopage wp]{wpFunctionTheory2019a} Both methods can be expressed by
\hlyn: Insert the respective symbol as first argument of the \hlyn\ basic
functions. If you don't need any additional specifier, add at least an empty
attribution list \texttt{\#'()}:
...
\subsection{Indicating a Special Bass}

Usually the bass note of a described chord is only revealed if it is not the
root of the chord. Due to the fact, that in the 'scale-step-theory' the step
number of a chord refers by definition its bass, the scale-step-theory does not
use bass notes. For indicating the bass note expand the attribute list by the
...
\subsection{Indicating other descant tones}

In accordance to the method for writing a basso continuo, the scale-step-theory
must indicate deviations from the default case by using additional numbers: if
the upper voices use other tones than those which are automatically and
defaultly referred by the third or fifth or eigth upon the tone referred by the
step number, then one must add the numbers of these other tones.
