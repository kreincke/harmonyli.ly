...
A preliminary result of this investigation was, that we had three back-ends for
...
in an unacceptable manner. So, I started a re-implementation of \acc{harmony} for
\acc{LilyPond} by using its LISP based language \acc{GUILE} and the respective
...
cadence. The representation of the harmonic analysis does not give him any
...
Finally, some of you might have noted, that the closure of an intermediary zoom
row sometimes is a bit ugly (\texttt{bar 6-9}). Let us shortly explain the
inevitable reasons before we present you a less ugly workaround:
...
embedded into a markup-command is mapped to the next chord of the voice to which
the analysis has been bound. And intermediary row on the other side is indicated
by an open bracket as prefix and a closed bracket as suffix. From the viewpoint
of \lily, the connection of the symbol and its prefix resp. its suffix is the
symbol itself. And in a lyric stream \lily\ only allows us to underscore the
symbol as whole, not only the half of the symbol or the part before the closing
bracket. That's the reason, why in this case the line in \texttt{bar 9} crosses
the bracket.

But note, this will become only visible in case of very long notes as we used
here. If you nevertheless meet such unattractive figures, you can use the
following workaround:

\begin{itemize}
  \item Use the method \acc{Binding \has{s} to a Dedicated 'Analysis'
  Staff}\footnote{\ra S. \pageref{BindingToDedicatedAnalysisStaff}}
  \item Split the long note into a large note and a shorter note.
  \item stop the text span before the shorter note.
  \item close the intermediary row by refering to the shorter note.
\end{itemize}
