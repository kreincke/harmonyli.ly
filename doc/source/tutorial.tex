...
respective analysis symbols: it prevents horizontal overlappings, even if a
\has\ is much longer than the respective note.

The little disadvantage is that \hlyn\ requires a dedicated voice to which the
row of \has{s} can be bound. After having set up your \lily\ file as described
above you have three opportunities to fulfill this requirement:

\subsection{Binding \has{s} to a musical voice}
...
explain, how particular \has{s}\ are generated by \hlyn\ commands. For that
purpose, \hlyn\ offers two interfaces: the basic \hlyn-functions and some
often used instantiations. This chapter describes general interface: 
...
purpose of the context. Up to that point we will ignore this opportunity in our
examples.} into an intermediary chord, you only must replace the function
\texttt{\textbackslash setHas} by the function \texttt{\textbackslash setImHas}.
...
\score {
  ...
  \addlyrics { 
    \markup \setHas "T" #'(("B"."3")("a"."7♮")("b"."9")) 
    \markup \setHas "III" #'(("a"."5♮")("b"."7")) 
  }
  \layout { \context { \Lyrics \consists "Text_spanner_engraver" } }
  \midi {}
}
...
In the functional harmony theory, by default each function refers to the keynote
of the piece: A major is the dominant of D major, but the subdominant of E
major. But sometimes, the musicologist wants to indicate that a chord has a
function with respect to the root of the succeeding chord. These kind of chords
...
Using intermediary chords in explicit descriptions leads directly to the
conclusion, that the syntax of the functional harmony theory is not sufficiently
elaborated and that therefore it usages still needs an underlying 'good will'
understanding of its reader. The deceptive cadence can easily prove this
statement. Traditionally we descibe a deceptive cadence like that:
...
The disadvantage of that notation is, that the reader has to know, that A major
...
It is better to explicate such implicit interpretations by an expanded language. 
\hlyn\ offers you the opportunity to express such relationships explicitly by
indicating the context of a chord:
...
Here you can interpret the musical functions only by using the 
definitions explicitly given by the functional harmony theory. You do not
need any additional explanations:
\begin{itemize}
  \item The \texttt{D} leads to the tonic.
  \item An intermediary chord \texttt{()} refers to the successor.
  \item The succrssor is not the tonic, but the relative tonic.
